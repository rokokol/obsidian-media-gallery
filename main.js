...
var parseExplicitBlock = (app2, container, src, sourcePath) => {
  ...
  const overrides = {};
  const imageLines = [];
  const knownKeys = /* @__PURE__ */ new Set(["type", "radius", "gutter", "sortby", "sort", "mobile", "columns", "height", "path", "fit", "waveform", "spectrogram"]);
  lines.forEach((line) => {
...
var getImagesList = (plugin, app2, container, settings) => {
  ...
  const mediaFiles = files.filter((file) => isVaultMedia(file));
  ...
  onload() {
    return __async(this, null, function* () {
      ...
      if (explicitImages.length) {
        ...
        this._imagesList = explicitImages;
      } else {
...
var ImgGallery = class extends import_obsidian5.Plugin {
  ...
  getCachedMediaFiles(path) {
    ...
    const normalizedPath = (0, import_obsidian5.normalizePath)(path);
    if (this._cachedMediaFolders.has(normalizedPath))
      return this._cachedMediaFolders.get(normalizedPath);
    const prefix = `${normalizedPath}/`;
    const filtered = this._cachedMediaFiles.filter((file) => file.path.startsWith(prefix));
    this._cachedMediaFolders.set(normalizedPath, filtered);
    return filtered;
  }
